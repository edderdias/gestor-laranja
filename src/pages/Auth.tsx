        ...
        <CardContent className="p-0 flex flex-col md:flex-row">
          <div className="w-full md:w-[30%] p-8 md:py-24 md:px-10 bg-[#2C7F24] flex flex-col justify-center border-b md:border-b-0 md:border-r border-slate-100">
            <div className="w-full space-y-8">
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                {mode === "signup" && (
                  <div className="space-y-2">
                    ...
                    <Input id="fullName" name="fullName" placeholder="Digite seu nome" required disabled={isLoading} className="bg-white text-slate-900" />
                  </div>
                )}
                <div className="space-y-2">
                  <Label htmlFor="email" className="text-white/90">Seu e-mail</Label>
                  <Input id="email" name="email" type="email" placeholder="Digite seu e-mail" required disabled={isLoading} className="bg-white text-slate-900" />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="password" className="text-white/90">Sua senha</Label>
                  <Input id="password" name="password" type="password" placeholder="Digite sua senha" required disabled={isLoading} className="bg-white text-slate-900" />
                </div>
              ...
              <div className="text-center space-y-2">
                <button type="button" onClick={() => setMode(mode === "signin" ? "signup" : "signin")} className="text-sm text-white/80 hover:text-white hover:underline">
                  {mode === "signin" ? "Não tem uma conta? Registre-se" : "Já tem uma conta? Faça login"}
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="w-full md:w-[70%] bg-white flex items-center justify-center p-4 md:py-20 md:px-12">
            <div className="relative w-full max-w-4xl flex items-center justify-center">
              <img src="/método certo.png" alt="Método Certo" className="w-full h-auto object-contain" />
            </div>
          </div>
        </CardContent>
      </Card>
